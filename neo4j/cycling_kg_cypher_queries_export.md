# Cycling Infrastructure KG — Cypher Query Export

This file collects the Cypher queries used for the Neo4j AuraDB knowledge graph, including validation, schema update, accident severity modelling, research-question queries, SQL/Cypher comparison queries, and visualisation support.
...
Expected counts after import, before adding severity:
...
After adding `HAS_SEVERITY`, expected additional relationship:
...
Use this only if accidental zero-count severity nodes were created, such as `Letsel`, `UMS`, or `Materieel`.
...
## 13. RQ2 — Accident hotspots with severity breakdown

This version uses the semantic `AccidentSeverity` node and `HAS_SEVERITY` relationship.
...
## 15. RQ3 — Formal bike parking capacity context
...
DS1 is used as supplementary context because it only covers formally registered bike parking facilities.
...
## 16. RQ4 / Main RQ — Combined priority candidates
...
This is the main combined query. It uses accident risk, bike lane density, and formal bike parking context.
...
## 17. RQ5 — Dense areas with infrastructure gap
...
This query uses density context and infrastructure indicators. It prioritises dense neighbourhoods with lower bike lane density and lower formal parking capacity context.
...
# SQL vs Cypher comparison support

## 18. Warning example — fan trap style query in Cypher

This query is intentionally not used for final analysis. It demonstrates that row multiplication can also happen in Cypher if multiple one-to-many patterns are matched before aggregation.
...
Correct Cypher avoids this by aggregating each relationship pattern step by step using `WITH`, as shown in Query 16.

---

# Visualisation / example graph queries

## 19. Select a readable example neighbourhood

This query helps find a neighbourhood with accidents, bike parking, and bike lane segments for a readable instance graph.
...
Use this to generate a readable Neo4j instance graph around Binnenstad. It limits each type of connected node.
...
Use this if the poster should show the semantic severity node.
...
# Display names for cleaner Neo4j visualisation

## 22. Add display names to Neighbourhood nodes
...
## 23. Add display names to CyclingAccident nodes
...
## 24. Add display names to BikeParkingFacility nodes
...
## 25. Add display names to BikeLaneSegment nodes
...
## 26. Add display names to AccidentSeverity nodes
...
Use this only if you need to undo the severity schema update.
